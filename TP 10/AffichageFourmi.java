...
public class AffichageFourmi extends JFrame {
    ...
    class PanneauGrille extends JPanel {
        ...
        public PanneauGrille(Plateau simul, int maRes) {
            res = maRes;
            worldImage = new BufferedImage(res*simul.etat[0].length,res*simul.etat.length,BufferedImage.TYPE_INT_RGB);
            // System.out.println(new Dimension(res*monde[0].length,res*monde.length));
            setPreferredSize(new Dimension(res*simul.etat[0].length,res*simul.etat.length));
        }
        private void dessineMonde(Graphics g) {
            int nbL = simulation.etat.length;
            int nbC = simulation.etat[0].length;
            // couleur de fond
